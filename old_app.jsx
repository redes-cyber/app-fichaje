...
function App() {
  ...
  const handleEntrada = async () => {
    ...
    try {
      ...
      // Guardar sesi├│n activa localmente
      const session = {
      ...
      // A├▒adir al historial local de forma optim├¡stica
      setSessions(prev => [{
        empleado: employeeName,
        accion: 'Jornada',
        fecha: fechaStr,
      ...
      mostrarEstado('Ô£à Entrada fichada correctamente');
    } catch (err) {
      mostrarEstado('ÔÜá´©Å ' + err.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSalida = async () => {
    ...
    if (!currentSession) {
      mostrarEstado('ÔÜá´©Å No hay una entrada activa registrada.', 'error');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      mostrarEstado('­ƒö┤ Salida fichada. Total: ' + totalHoras + ' horas');
    } catch (err) {
      mostrarEstado('ÔÜá´©Å ' + err.message, 'error');
    } finally {
      setLoading(false);
